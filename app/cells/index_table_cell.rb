class IndexTableCell < Cell::ViewModel
  def show
    render :table_framework
  end
  ...
  def rows
    cell 'index_table_cell/row', {collection: index_objects}, @options
  end
  ...
  class RowCell < IndexTableCell
    ...
    def data_content(field)
      cell('index_table_cell/column', model.send(field), @options).()
        .truncate(40)
    end
  ...
  class ColumnCell < IndexTableCell
  ...
  class BooleanColumnCell < IndexTableCell
    def show
  ...
  class CollectionColumnCell < IndexTableCell
    def show
      model.map do |element|
        cell('index_table_cell/association_column', element)
      end.join(', ')
    end
  end

  class AssociationColumnCell < IndexTableCell
    def show
