          ...
          proc do
            if request.get? # NEW
              ...
              if @object.save
                ...
                # NOTE: Hacky hack hack: retrieve
                # TargetAudienceFiltersOffers if object is a cloned offer
                regex = request.referer.match(%r{/offer/(\d+)/clone})
                if regex && regex[1] && Offer.where(
                  id: regex[1]
                ).any? && @object.class == Offer
                  ...
                  source_offer.target_audience_filters_offers.map do |tafo|
                    ...
                    @object.target_audience_filters_offers <<
                      TargetAudienceFiltersOffer.new(attributes)
                  end
                end
                respond_to do |format|
                  format.html { redirect_to_on_success }
                  format.js   do
                    render json: { id: @object.id.to_s,
                                   label: @model_config.with(
                                     object: @object
                                   ).object_label }
                  end
                end
