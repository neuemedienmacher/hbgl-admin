...
function processAssociations(associations, modelInstance, entities, model) {
  ...
  associations.map(([assoc_name, assoc]) => {
    ...
    let href = ''
    if (
      settings.index[entityName] &&
      settings.index[entityName].member_actions
    ) {
      href = `/${entityName}/`
    }
...
function gatherAssociatedItems(assocName, entityName, entities, modelInstance) {
  ...
  if (!associatedIds.length || !entities[entityName]) {
    return []
  }
  return filter(entities[entityName], (entity) =>
    associatedIds.includes(entity.id)
  )
}
