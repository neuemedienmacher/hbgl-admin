...
function buildActionButtonData(
  ...
  const changes =
    (formData && formData._changes && formData._changes.length) ||
    hasAtLeastOneSubmodelForm(formData)
...
const mapStateToProps = (state, ownProps) => {
  const { model, id, submodelKey, modifySeedData, formIdSpecification } =
    ownProps
  ...
  const afterSaveActions = mapCollection(
    settings.AFTER_SAVE_ACTIONS,
    (value, key) => ({
      action: key,
      name: value,
      active: afterSaveActiveKey === key,
    })
  )
...
const mergeProps = (stateProps, dispatchProps, ownProps) => {
  ...
  const resetForm = (changes, response) => {
    ...
    const desiredFormData = seedDataFromEntity(
      entity,
      stateProps.formObjectClass,
      ownProps.modifySeedData
    )
  ...
  return {
    ...
    afterError(_formId, response) {
      ...
      dispatch(addFlashMessage('error', 'Es ist ein Serverfehler aufgetreten.'))
      response
        .text()
        .then((errorMessage) =>
