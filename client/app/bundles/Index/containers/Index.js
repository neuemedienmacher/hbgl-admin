...
const mapStateToProps = (state, ownProps) => {
  ...
  const optional = ownProps.identifierAddition
    ? `_${ownProps.identifierAddition}`
    : ''
  ...
  if (!isLoading && state.ajax[identifier]) {
    const perPage = state.ajax[identifier].meta.per_page
    const startValue = (state.ajax[identifier].meta.current_page - 1) * perPage
    const totalEntries = state.ajax[identifier].meta.total_entries
    const toValue = Math.min(startValue + perPage, totalEntries)
