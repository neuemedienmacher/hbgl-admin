...
const mapStateToProps = (state, ownProps) => {
  ...
  const data =
    (state.entities.count &&
      state.entities.count[ownProps.model] &&
      state.entities.count[ownProps.model][columnName] &&
      state.entities.count[ownProps.model][columnName][selectedCity || ALL]) ||
    {}
  const columnElements =
    (state.ajax[columnName] &&
      state.ajax[columnName].data.map((datum) => datum.attributes.name)) ||
    []
  const loadedCities =
    (state.entities.count &&
      state.entities.count[ownProps.model] &&
...
const mergeProps = (stateProps, dispatchProps, ownProps) => {
  ...
  const dispatchDataLoad = function (columnName, team, element, cityId) {
    dispatch(
      loadAjaxData(
        ...
        'lastData',
        {
          transformer: entryCountGrabberTransformer(
            columnName,
            team,
            element,
            cityId
  ...
  const loadData = function (columnName, teams, columnElements, cityId) {
    for (let team of teams.concat(null)) {
  ...
  return {
    ...
    loadColumns(columns) {
      dispatch(loadAjaxData(columns, {}, columns))
    },
    ...
    onCityChange(selected) {
      ...
      if (!stateProps.loadedCities.includes(city)) {
        loadData(
          stateProps.columnName,
          stateProps.teams,
          stateProps.columnElements,
          city
        )
      }
    },
