...
function involvementCount(assignments, userID) {
  ...
  const assignmentCount = valuesIn(assignments).filter(assignment => (
...
const mapStateToProps = (state, ownProps) => {
  ...
  const assignableChanged = assignableFormChanges && assignableFormChanges.length;
  ...
  const settingsActions = settings.index[model]["assignment-actions"] ||
