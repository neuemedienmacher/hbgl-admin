...
const mapStateToProps = (state, ownProps) => {
  let optional = ownProps.identifierAddition
  ...
  const count = state.ajax[identifier]
    ? state.ajax[identifier].meta.total_entries
    : 0
...
const mergeProps = (stateProps, dispatchProps, ownProps) => ({
  ...
  loadData(
    query = merge(clone(ownProps.optionalParams), ownProps.lockedParams),
    nextModel = ownProps.model
  ) {
    let optional = ownProps.identifierAddition
      ? '_' + ownProps.identifierAddition
      : ''
