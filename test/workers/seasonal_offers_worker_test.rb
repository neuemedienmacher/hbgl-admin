...
class SeasonalOffersWorkerTest < ActiveSupport::TestCase # to have fixtures
  ...
  it 'approves a seasonal_pending off that has entered its TimeFrame '\
    ...
    starts_today = FactoryBot.create :offer, aasm_state: 'seasonal_pending',
                                             starts_at: today,
                                             ends_at: today + 90.days
    starts_later = FactoryBot.create :offer, aasm_state: 'seasonal_pending',
  ...
  it 'it creates an AsanaTask for paused offers that start in 30 days '\
    ...
    today = Time.zone.today
    starts_soon = FactoryBot.create :offer, aasm_state: 'paused',
                                            starts_at: today + 30.days,
                                            ends_at: today + 90.days
    starts_later = FactoryBot.create :offer, aasm_state: 'paused',
  ...
  it 'it should set seasonal offers that left their TimeFrame to paused '\
     'but ignore non-seasonal offers' do
    today = Time.zone.today
    Timecop.freeze(today - 1.day)
    normal_offer = FactoryBot.create :offer, aasm_state: 'approved',
                                             ends_at: today
    seasonal_offer = FactoryBot.create :offer, aasm_state: 'approved',
                                               starts_at: today - 30.days,
                                               ends_at: today
    Timecop.return
