...
class API::V1::FieldSet::RepresenterTest < ActiveSupport::TestCase
  ...
  it 'should correctly process the class_name' do
    ...
    result['associations'][:created_assignments]['class-name']
      .must_equal 'assignments'
  end
  ...
  it 'should provide the correct associations for User class' do
    ...
    result['associations'][:user_team_observing_users]['key']
      .must_equal ['user_id']
  ...
  it 'should provide the correct associations for Organization class' do
    ...
    result['associations'][:website]['key']
      .must_equal [''] # ignores belongs_to to associations
    result['associations'][:contact_people]['key']
      .must_equal ['organization_id']
  ...
  it 'should provide the correct associations for Division class' do
    ...
    result['associations'][:assignments]['key']
      .must_equal %w[assignable_id assignable_type]
    result['associations'][:organization]['key']
      .must_equal [''] # ignores belongs_to to associations
    result['associations'][:section]['key']
      .must_equal [''] # ignores belongs_to to associations
    result['associations'][:city]['key']
      .must_equal [''] # ignores belongs_to to associations
    result['associations'][:area]['key']
      .must_equal [''] # ignores belongs_to to associations
    result['associations'][:divisions_presumed_tags]['key']
      .must_equal ['division_id']
    result['associations'][:presumed_tags]['key']
      .must_equal ['presuming_divisions.id'] # TODO?!
    result['associations'][:divisions_presumed_solution_categories]['key']
      .must_equal ['division_id']
    result['associations'][:presumed_solution_categories]['key']
      .must_equal ['presuming_divisions.id']
