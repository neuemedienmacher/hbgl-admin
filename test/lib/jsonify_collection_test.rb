...
class JsonifyCollectionTest < ActiveSupport::TestCase
  ...
  it 'calls #for_collection on the representer and adds meta information' do
    collection = Category.paginate(page: 2, per_page: 1)
    params = {
      'controller' => 'controller', 'action' => 'action',
      'format' => 'xml-csv', 'foo' => 'bar'
    }
    result =
      subject.call(API::V1::Category::Representer::Show, collection, params)
    result.class.must_equal String
    result_hash = JSON.parse result
    result_hash['data'].class.must_equal Array
    result_hash['data'].first['id'].must_equal '2'
    result_hash['meta']['total_entries'].must_equal 4
    result_hash['meta']['total_pages'].must_equal 4
